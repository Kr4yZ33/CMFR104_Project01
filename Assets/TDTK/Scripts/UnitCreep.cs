...
namespace TDTK{
	...
	public class UnitCreep : Unit {
		...
		public override void FixedUpdate(){
			...
			if(IsTurret()){
				if(stopToAttack){
					if((attackTarget!=null && !CreepIsOnAttackCD()) || Time.time-lastAttackTime<0.5f) speedMultiplier=0; 
					else{
						speedMultiplier=1;
						if(attackTarget==null && !resetingAim) StartCoroutine(ResetAim());
					}
				}
				else{
					if(attackTarget==null && !resetingAim) StartCoroutine(ResetAim()); 
		...
		void CreepFunction(){
			...
			if(IsSupport()){
				...
				for(int i=0; i<supportTgtList.Count; i++){
					if(Vector3.Distance(GetPos(), supportTgtList[i].GetPos())<=range) continue;
			...
			if(dist<0.05f){
				...
				if(!reverse){
					...
					if(subWpIdx>=subPath.Count){
						...
						if(path.GetWP(wpIdx).Count>0) subPath=path.GetWP(wpIdx, EnableBypass());
					}
				}
				else{
				...
				thisT.Translate(dir*Mathf.Min(currentSpeed*Time.fixedDeltaTime, dist), Space.World);
			}
		...
		//for target scanning
		public override float GetPathDist(){ return path.GetDistanceToEnd(); }
