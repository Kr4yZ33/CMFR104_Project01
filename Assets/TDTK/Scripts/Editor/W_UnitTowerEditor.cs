...
namespace TDTK {
	...
	public class UnitTowerEditorWindow : TDEditorWindow {
		...
		private Vector2 DrawUnitConfigurator(float startX, float startY, UnitTower unit){
			...
			unit.limitInScene=EditorGUI.DelayedIntField(new Rect(startX+spaceX+12, startY, widthS, height), unit.limitInScene);	GUI.color=Color.white;
		...
		protected float DrawTowerStats(float startX, float startY, UnitTower unit){
			...
			TDE.Label(startX, startY+=spaceY, width, height, "Unlock Via Perk:", "");
			...
			for(int i=0; i<unit.statsList.Count; i++){
				...
				if(unit.IsTurret()) tType=_EType.TTurret;
					...
					if(unit.destroyOnTrigger) tType=_EType.TMine;
					else tType=_EType.TMineCD;
				}
		...
		protected float DrawGeneralSetting(float startX, float startY, UnitTower unit){
			...
			if(unit.IsMine()){
				TDE.Label(startX, startY+=spaceY, width, height, "Destroy On Trigger:", "Check to have the mine self destruct after being triggered\nOtherwise the mine can be triggered repetitively when it's not on cooldown");
				unit.destroyOnTrigger=EditorGUI.Toggle(new Rect(startX+spaceX, startY, width, height), unit.destroyOnTrigger);
			}
