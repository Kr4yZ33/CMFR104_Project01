...
namespace TDTK {
	...
	public class TDEditorWindow : EditorWindow{
		...
		protected float DrawUnitSetting(float startX, float startY, Unit item){
			...
			if(item.IsAOE() || item.IsResource() || item.IsMine()){
				startY+=10;
				
				TDE.Label(startX, startY+=spaceY, width, height, "Shoot Effect:", "OPTIONAL - The object to be spawned on shoot-point on every countdown");
				item.shootEffectObj=(GameObject)EditorGUI.ObjectField(new Rect(startX+spaceX, startY, width, height), item.shootEffectObj, typeof(GameObject), true);
				
				TDE.Label(startX, startY+=spaceY, width, height, "Effect Duration:", "Duration (in second) before the effect is destroyed");
				item.shootEffectDuration=EditorGUI.DelayedFloatField(new Rect(startX+spaceX, startY, widthS, height), item.shootEffectDuration);
			}
			if(item.IsTurret()){
				startY+=10;
		...
		#region draw stats
		public enum _EType{ TTurret, TAOE, TSupport, TRsc, TMine, TMineCD, TBlock, CDefault, CTurret, CAOE, CSupport, CSpawner, Ability, PerkT, PerkA, PerkE, Effect,}
		
		public static bool IsTower(_EType type){ return (int)type<=(int)_EType.TBlock; }
		public static bool IsCreep(_EType type){ return (int)type>=(int)_EType.CDefault & (int)type<=(int)_EType.CSpawner; }
		...
		public static bool UseDamage(_EType t){ return t==_EType.TTurret | t==_EType.TAOE | IsMine(t) | t==_EType.CTurret | t==_EType.CAOE; }
		...
		public static bool UseAOERange(_EType t){ return t==_EType.TTurret | IsMine(t) | t==_EType.CTurret; }
		public static bool UseHitNCrit(_EType t){ return t==_EType.TTurret | t==_EType.TAOE | IsMine(t) | t==_EType.CTurret | t==_EType.CAOE; }
		public static bool UseRscGain(_EType t){ return t==_EType.TRsc; }
		public static bool UseOnHitEffect(_EType t){ return t==_EType.TTurret | t==_EType.TAOE | t==_EType.TSupport | IsMine(t) | t==_EType.CTurret | t==_EType.CAOE | t==_EType.CSupport; }
		public static bool UseOnHitEffectChance(_EType t){ return (!IsTower(t) && !IsCreep(t)) | t==_EType.TTurret | t==_EType.TAOE | IsMine(t) | t==_EType.CTurret | t==_EType.CAOE; }
		
		public static bool IsMine(_EType t){ return t==_EType.TMine | t==_EType.TMineCD; }
		...
		public static float DrawStats(float startX, float startY, Stats item, _EType type, bool compressWidth=false){
			...
			else if(type==_EType.TMineCD || type==_EType.TMine){
				GUI.color=(type==_EType.TMine ? Color.grey : Color.white);
				TDE.Label(startX, startY+=spaceY, width, height, "Cooldown:", "");
				item.cooldown=EditorGUI.DelayedFloatField(new Rect(startX+spaceX, startY, widthS, height), item.cooldown);
			...
			if(!IsEffect(type) && (UseOnHitEffect(type) || IsAbility(type) || IsPerkA(type) || IsPerkT(type))){
				...
				for(int i=0; i<item.effectOnHitIDList.Count; i++){
					...
					int effIdx=Effect_DB.GetPrefabIndex(item.effectOnHitIDList[i]);		bool removeEff=false;
					...
					if(!compressWidth){
						...
						effIdx = EditorGUI.Popup(new Rect(startX+spaceX-widthS*0.5f+3, startY, widthS*2.5f, height), effIdx, Effect_DB.label);
						if(GUI.Button(new Rect(startX+spaceX-widthS+5, startY, height, height), "-")){ item.effectOnHitIDList.RemoveAt(i); removeEff=true; }
					}
					
					if(effIdx>=0 && !removeEff) item.effectOnHitIDList[i]=Effect_DB.GetItem(effIdx).prefabID;
					
					startY+=spaceY;
				}
				
				int newEffID=-1;
				if(!compressWidth)	newEffID = EditorGUI.Popup(new Rect(startX+spaceX, startY, width, height), newEffID, Effect_DB.label);
				else	newEffID = EditorGUI.Popup(new Rect(startX+spaceX-widthS*0.5f+3, startY, widthS*2.5f, height), newEffID, Effect_DB.label);
				...
				//~ TDE.Label(startX, startY+=spaceY, width, height, "Effect On Hit:", "Offense/AOE Unit/Ability: The effect applies to target when an attack hit\nSupport Unit: The effect applies to friendly target");
				//~ int effIdx=Effect_DB.GetPrefabIndex(item.effectOnHitID);
				...
				//~ if(effIdx>=0 && !removeEff) item.effectOnHitID=Effect_DB.GetItem(effIdx).prefabID;
			...
			if(UseOnHitEffectChance(type)){
				...
				if(useEffOnHit && item.effectOnHitIDList.Count<=0 && !(IsPerkA(type) || IsPerkT(type))) GUI.color=Color.grey;
