	...
	public class UnitTower : Unit {
		...
		void TowerFunction(){
			...
			if(IsMine()){// && cooldownAttack<0){
				...
				if(tgtList.Count>0){
					...
					if(destroyOnTrigger) Destroyed(true, false);
				}
				else cooldown=0.1f;//cooldownAttack=0.1f;
			}
		}
		
		void FireMiscEffect(){ 
			if(shootEffectObj==null) return;
			StartCoroutine(_FireMiscEffect());
		}
		IEnumerator _FireMiscEffect(){
		...
		public void Upgrade(int upgradeIdx=0){
			if(GetUpgradeType()==0){
				...
				TowerManager.BuildTower(upgradeTowerList[upgradeIdx], buildPlatform, nodeID, true, false);
				Destroy(thisObj);
			}
		}
