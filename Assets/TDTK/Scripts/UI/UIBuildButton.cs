...
namespace TDTK{
	...
	public class UIBuildButton : UIScreen {
		...
		private void AddBuildButton(int idx, Sprite icon, string txt){
			...
			if(UIControl.InTouchMode() && UIControl.UsePointNBuildMode()) buildButtons[idx].SetCallback(null, null, this.OnBuildButton, null);
			else buildButtons[idx].SetCallback(this.OnHoverBuildButton, this.OnExitBuildButton, this.OnBuildButton, null);
			
			//if(UIControl.InTouchMode() && UIControl.UsePointNBuildMode()) buildButtons[idx].SetCallback(null, null, null, null);
			//else buildButtons[idx].SetCallback(this.OnHoverBuildButton, this.OnExitBuildButton, null, null);

			//buildButtons[idx].button.onClick.AddListener(delegate { OnBuildButton(buildButtons[idx].rootObj); });
			//buildButtons[idx].SetCallback(this.OnHoverBuildButton, this.OnExitBuildButton, this.OnBuildButton, null);
			
			if(icon!=null) buildButtons[idx].imgMain.sprite=icon;
			else buildButtons[idx].imgMain.enabled=false;
		...
		public void _ReplaceBuildable(int idx, UnitTower tower){
			...
			buildButtons[idx].imgMain.sprite=tower.icon;
			buildButtons[idx].lbMain.text=tower.GetCost()[0].ToString("f0");
		}
		...
		void UpdateDisplay(){
			for(int i=0; i<buildButtons.Count; i++){
				if(i<sInfo.buildableList.Count){
					buildButtons[i].imgMain.sprite=sInfo.buildableList[i].icon;
					//buildButtons[i].lbMain.text=sInfo.buildableList[i].unitName;
					buildButtons[i].lbMain.text=sInfo.buildableList[i].GetCost()[0].ToString("f0");
